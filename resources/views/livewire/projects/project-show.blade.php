<div class="app-content mx-auto flex w-full max-w-4xl flex-col gap-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2 sm:shrink-0">
            ...
            <flux:button
                ...
                variant="ghost"
                icon="document-text"
                :href="route('project.docs', $this->project)"
                wire:navigate
                data-test="project-docs-link"
            >
                {{ __('Docs') }}
            </flux:button>
            <flux:button
                ...
                variant="primary"
                icon="view-columns"
                :href="route('project.board', $this->project)"
                wire:navigate
            >
                {{ __('Board') }}
            </flux:button>
                ...
                <flux:dropdown align="end">
                    ...
                    <flux:menu>
                        @can('manage-tags', $this->project)
                            <flux:menu.item
                                icon="tag"
                                :href="route('project.tags', $this->project)"
                                wire:navigate
                                data-test="manage-tags-link"
                            >
                                {{ __('Manage tags') }}</flux:menu.item>
                        @endcan
                        @can('manage-variables', $this->project)
                            <flux:menu.item
                                icon="variable"
                                :href="route('project.variables', $this->project)"
                                wire:navigate
                                data-test="manage-variables-link"
                            >
                                {{ __('Manage variables') }}</flux:menu.item>
                        @endcan
                        @can('manageSettings', $this->project)
                            <flux:menu.item
                                icon="bookmark-square"
                                :href="route('project.task-types', $this->project)"
                                wire:navigate
                                data-test="manage-task-types-link"
                            >
                                {{ __('Manage types') }}</flux:menu.item>
                            <flux:menu.item icon="pencil-square" wire:click="edit" data-test="edit-project">
                                {{ __('Edit') }}</flux:menu.item>
                        @endcan
                        @can('manageMembers', $this->project)
                            <flux:menu.item
                                icon="users"
                                wire:click="$set('managingMembers', true)"
                                data-test="manage-members"
                            >
                                {{ __('Manage members') }}</flux:menu.item>
                        @endcan
    ...
    <div class="flex flex-col gap-3">
        <div class="flex items-center justify-between gap-3">
            ...
            >
                <flux:icon
                    :name="$tasksCollapsed ? 'chevron-right' : 'chevron-down'"
                    variant="micro"
                    class="text-zinc-400"
                />
                ...
                <flux:badge
                    size="sm"
                    color="zinc"
                    data-test="open-task-count"
                >{{ $this->openTasks->count() }}</flux:badge>
            </button>
                    ...
                    <flux:dropdown align="start">
                        ...
                        <flux:popover class="flex max-h-[28rem] w-72 flex-col gap-4 overflow-y-auto">
                            ...
                            <flux:field>
                                ...
                                <flux:checkbox.group
                                    wire:model.live="priorityFilters"
                            ...
                            @if ($this->members->isNotEmpty())
                                <flux:field>
                                    ...
                                    <flux:checkbox.group
                                        wire:model.live="assigneeFilters"
                ...
                <div class="flex flex-col gap-3">
                    @forelse ($this->openTasks as $task)
                        <x-root-task-card
                            :task="$task"
                            :short-name="$this->project->short_name"
                            :can-archive="$this->canArchiveTask"
                ...
                @if ($showArchived && $this->archivedTasks->isNotEmpty())
                    <div class="flex flex-col gap-3" data-test="archived-tasks">
                        <flux:heading
                            size="sm"
                            class="text-zinc-500 dark:text-zinc-400"
                        >{{ __('Archived tasks') }}</flux:heading>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex min-w-0 flex-col gap-0.5">
                                ...
                                <flux:text
                                    size="xs"
                                    class="text-zinc-400"
                                >{{ __('by :name', ['name' => $note->user?->name ?? __('Deleted user')]) }}</flux:text>
                            </div>
                                ...
                                <flux:dropdown align="end">
                                    ...
                                    <flux:menu>
                                        ...
                                        <flux:menu.item
                                            ...
                                            data-test="unshare-note-{{ $note->id }}"
                                        >
                                            {{ __('Make private') }}</flux:menu.item>
                                        <flux:menu.separator />
                                        <flux:menu.item
                                            icon="trash"
                                            variant="danger"
                                            ...
                                            data-test="delete-public-note-{{ $note->id }}"
                                        >
    ...
    @can('manageMembers', $this->project)
        <flux:modal wire:model="managingMembers" class="md:w-xl" data-test="members-modal">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1" data-test="members-list">
                    @foreach ($this->members as $member)
                                        ...
                                        <flux:dropdown position="bottom" align="end">
                                            ...
                                            <flux:menu>
                                                @foreach ($row['addable'] as $assignable)
                                                    <flux:menu.item
                                                        wire:click="addMemberRole({{ $member->id }}, '{{ $assignable->name }}')"
                                                        data-test="add-member-role-{{ $member->id }}-{{ $assignable->name }}"
                                                    >
                                                        {{ $this->roleLabel($assignable->name) }}</flux:menu.item>
                                                @endforeach
                                            </flux:menu>
                                        </flux:dropdown>
                                    @endif
    ...
    @can('manage-variables', $this->project)
        <livewire:variables.create-variable :short-name="$this->project->short_name" />
    @endcan
</div>
