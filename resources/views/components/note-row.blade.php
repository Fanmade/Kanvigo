        ...
        <div class="flex flex-wrap items-center gap-1.5">
            ...
            @if ($note->convertedTask)
                ...
                >
                    ...
                    >{{ __('Converted → :ref', ['ref' => $note->convertedTask->reference]) }}</flux:badge>
                </a>
            @endif
        </div>
    </div>
    ...
    <div class="flex shrink-0 items-center gap-0.5">
        ...
        <flux:dropdown align="end">
            <flux:button
                ...
                data-test="note-actions-{{ $note->id }}"
            />
            <flux:menu>
                <flux:menu.item
                    icon="pencil-square"
                ...
                @unless ($note->convertedTask)
                    <flux:menu.item
                        icon="arrow-right-circle"
                        wire:click="$dispatch('open-create-task', { fromNoteId: {{ $note->id }} })"
                        data-test="convert-note-{{ $note->id }}"
                    >
                        {{ __('Convert to task') }}</flux:menu.item>
                @endunless
                @if ($note->project)
                    <flux:menu.item
